...
*   **`model`:** Modelo de Dados, contém as classes que representam as entidades de negócio e a lógica de dados. Classes principais: `Usuario`, `Alimento` (Abstrata), `Bebida`, `Comida`, `Pedido`, `PedidoItem`.
...
*   **`view`:** Interface do Usuário, contém as classes `JFrame` que definem a interface gráfica e a interação visual com o usuário. Classes principais: `Inicio`, `LoginUsuario`, `CadastroUsuario`, `Menu`, `BuscarAlimento`, `MeuPedidos`.
*   **`controller`:** Controlador, contém a lógica de negócio, mediando a interação entre a View e o Model. Classes principais: `ControleLogin`, `ControleCadastro`, `ControleMenu`.
*   **`dao`:** Acesso a Dados, responsável por toda a comunicação com o banco de dados. Classes principais: `Conexao`, `UsuarioDAO`, `AlimentoDAO`, `PedidoDAO`.
